...
def naive_softmax_loss_and_gradient(center_word_vec, outside_word_idx, outside_vectors, dataset):
    ...
    y_hat_copy[outside_word_idx] -= 1
    grad_center_vec = outside_vectors.T.dot(y_hat_copy)
    grad_outside_vecs = np.outer(y_hat_copy, center_word_vec)
    ### END CODE HERE
...
def skipgram(current_center_word, outside_words, word2ind, center_word_vectors, outside_vectors, dataset,
    ...
    for outside_word in outside_words:
        ...
        current_loss, current_grad_center, current_grad_outside = word2vec_loss_and_gradient(
            center_word_vectors[word2ind[current_center_word], :],
            outside_word_idx,  # This was missing!
            outside_vectors,
            dataset
        )
        loss += current_loss
        grad_center_vecs[word2ind[current_center_word], :] += current_grad_center
        grad_outside_vectors += current_grad_outside
    ### END CODE HERE
...
def word2vec_sgd_wrapper(word2vec_model, word2ind, word_vectors, dataset, window_size,
    ...
    for i in range(batchsize):
        ...
        c, gin, gout = word2vec_model(center_word, context, word2ind, center_word_vectors, outside_vectors, dataset,
